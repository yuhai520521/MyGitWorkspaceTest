...
//Global Variables
Uint16 response = 0xFF, ocr_contents[5], csd_contents[16], cid_contents[16];
Uint16 card_status[2], data_manipulation = TRUE, high_capacity = FALSE, crc_enabled = FALSE;

//######################### CHECK_CARD_INSERTION #############################
void sd_card_insertion(void)
{
	Uint16 i = 0;
// 卡槽的检测插入卡CD引脚没有连接IO中断
//	i = SDReg & 0X01;
//	while(i !=0) {};
